...
if [ ! -f "$WEB_ENV" ]; then
  echo "  ✗ Missing $WEB_ENV — copy from ShipSmart-Web/.env.example"
  ERRORS=$((ERRORS + 1))
else
...
if [ ! -f "$JAVA_ENV" ]; then
  echo "  ✗ Missing $JAVA_ENV — copy from ShipSmart-Orchestrator/.env.example"
  ERRORS=$((ERRORS + 1))
else
...
if [ ! -f "$PYTHON_ENV" ]; then
  echo "  ✗ Missing $PYTHON_ENV — copy from ShipSmart-API/.env.example"
  ERRORS=$((ERRORS + 1))
else
