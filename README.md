...
ShipSmart is split across five repos. Clone them as siblings under the same parent directory:

```
parent/
├── ShipSmart-Web/            React + Vite frontend
├── ShipSmart-Orchestrator/   Spring Boot Java API
├── ShipSmart-API/            FastAPI Python AI/advisory service
├── ShipSmart-MCP/            Standalone MCP tool server (FastAPI)
...
| ShipSmart-API | https://github.com/nia194/ShipSmart-API | `shipsmart-api-python` |
| ShipSmart-MCP | https://github.com/nia194/ShipSmart-MCP | `shipsmart-mcp` |
| ShipSmart-Infra | https://github.com/nia194/ShipSmart-Infra | — (no Render service) |

## Contents

- `supabase/` — migrations, edge functions (14), SQL snippets, `config.toml`. Latest migration `20260417120000_interview_upgrade.sql` adds optimistic-locking (`version`, `updated_at`), soft-delete (`deleted_at`), `status` columns, and new `idempotency_keys` / `audit_log` tables consumed by the Java Orchestrator (Flyway in validate mode mirrors this 1:1).
- `scripts/` — `dev-start.sh`, `check-env.sh`, `run-mcp-server.sh` (legacy: still cd's into `ShipSmart-API`; the MCP code now lives in `ShipSmart-MCP`), `verify-post-deployment.sh`, `build_study_guide.py`.
- `docs/` — topic-organized documentation:
  - **Snapshot docs (start here):** `architecture-summary.md` (current truth of the 5-service system), `gap-audit.md` (per-topic Implement / Partial / Reject verdicts), `implementation-plan.md` (Phase A→D delivery plan), `interview-upgrade-summary.md` (cross-repo change summary).
  ...
  - `architecture/` — system flow, AI-feature architecture, MCP tooling architecture, service boundaries, advisor flows, current-system-state, full-system-integration.
  ...
  - `roadmap/` — `OPTION-B1-START-HERE.md`, `OPTION-B1-AUTOMATION-INDEX.md`, interview/next-iteration roadmaps, phase history, migration checklist, known limitations, what-not-to-build, legacy edge functions.
...
4. Run `bash scripts/dev-start.sh all` to start the web/java/python services. The MCP server (`ShipSmart-MCP`) starts separately — `cd ../ShipSmart-MCP && uv run uvicorn app.main:app --reload --port 8001`.
...
- `ShipSmart-API/render.yaml` → 1 Python web service (FastAPI AI/advisory)
- `ShipSmart-MCP/render.yaml` → 1 Python web service (MCP tool server)
